...
syn match haskellTopLevelDecl "^\([_a-z][a-zA-z0-9_]*'*,\?\n*\s*\)\+::" contains=haskellTopLevelIdentifier,haskellOperators,haskellDelimiter
syn match haskellOtherDecl "^\s*\([_a-z][a-zA-z0-9_]*'*,\?\s*\)\+::" contains=haskellTopLevelIdentifier,haskellOperators,haskellDelimiter
"syn region haskellTopLevelDecl start="^\s*\([_a-z][a-zA-z0-9_]*'*,\?\s*\)\+" skip="\s" end="::" 
